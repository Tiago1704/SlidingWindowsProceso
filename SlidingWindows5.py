...
def MostrandoArrays(AS1, AS2):
    for i in zip(AS1):
        print(AS1[i])
    for j in zip(AS2):
        print(AS2[j])
        
def SliWin(AS1, AS2, TA1, TA2):
    for i in range(TA1):
        for j in range(TA2):
...
def MostrandoArraySliWindows(AS1, AS2, ASW, TA1, TA2):
    for i in range(TA1):
        for j in range(TA2):
            if ((AS1[i].all  == AS2[j].all)):
                ASW[i] = AS1[i]             
        ...
        """Luca, en este MostrandoArraySliWindows no te aparecerá nada porque no hay valores parecidos, no aparece nada porque
        son muchas celdas las que recorre hasta terminar, pero anda piola"""
